...
contract('TestCustomERC721Token', async accounts => {
    ...
    describe('match erc721 spec', function () {
        ...
        it('name()', async function () {
            var result = await config.customERC721Token.name.call();
            assert.equal(result, 'DOBEN', 'Could not get name.')
        })

        it('symbol()', async function () {
            var result = await config.customERC721Token.symbol.call();
        ...
        it('should return token url', async function () {
            ...
            var compare_string = 'https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/' + String(tokenId);
